...
export default function WorkTab() {
  ...
  useEffect(() => {
    ...
    const onReady = () => {
      console.log('[WorkTab] webview dom-ready')
    }
    const onConsole = (e: any) => {
      const methods = ['log', 'warn', 'error', 'info', 'debug'] as const
      const fn = methods[e.level] || 'log'
      console[fn](`[WorkTab:webview]`, e.message)
    }
    const onNavigated = () => {
      console.log('[WorkTab] webview did-stop-loading')
      if (!ready) setReady(true)
    }
    const onFail = (e: Event) => {
      console.error('[WorkTab] webview load failed:', (e as any).errorDescription)
    }
